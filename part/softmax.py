...
import argparse, datetime, random, os
import logging
...
parser.add_argument('--num-parts', type=int, default=6,
                    help='number of parts to use in last feature map.')
parser.add_argument('--epochs', type=str, default="10,90,120,140")
...
def validate(val_data, net, criterion, num_parts, ctx):
    ...
    for data, label in val_data:
        ...
        for i in range(opt.num_gpus):
            ...
            temp_loss = sum([criterion(X, label_list[i]) for X in outputs])/num_parts
            losses.append(temp_loss)
            temp_acc = sum([nd.mean(X.argmax(axis=1)==label_list[i].astype('float32')).asscalar() for X in outputs])/num_parts
    ...
    for epoch in range(epochs[-1]):
        ...
        for data, label in train_data:
            ...
            with autograd.record():
                losses = []
                for i in range(opt.num_gpus):
                    outputs = net(data_list[i])
                    temp_loss = sum([criterion(X, label_list[i]) for X in outputs])/(opt.num_parts+1)
                    losses.append(temp_loss)
        ...
        if val_data is not None:
            val_loss, val_accuray = validate(val_data, net, criterion, (opt.num_parts+1), ctx)
...
if __name__ == '__main__':
    ...
    net = resnet50(ctx=context, num_features=256, num_classes=751, num_parts=opt.num_parts)
